    ...
    <transition name="slide-up">
      <div class="setting-wrapper" v-show="ifSettingShow">
        ...
        <div class="setting-font-size">
          <div class="preview" :style="minFontSize">A</div>
          ...
          <div class="preview" :style="maxFontSize">A</div>
        </div>
      </div>
    </transition>
  </div>
...
export default {
  ...
  computed: {
    minFontSize () { // 最小字号
      let size = this.fontSizeList[0].fontSize
      let sizeStr = `{fontSize: ${size}px}`
      return sizeStr
    },
    maxFontSize () { // 最大字号
      let size = this.fontSizeList[length - 1].fontSize
      let sizeStr = `{fontSize: ${size}px}`
      return sizeStr
    }
  }
